...
def embed_documents(
    ...
    Args:
        ...
        provider: "cohere" (default, requires COHERE_API_KEY) or "openrouter" (requires OPENROUTER_API_KEY)
    ...
    Returns:
        List of dicts: {
            ...
            'metadata': dict with 'provider' showing actual method used after fallback chain,
...
def demo_semantic_search(embedded_chunks: List[Dict]):
    ...
    # (This ensures embedding dimensions match for similarity computation)
    query = "What is the company's remote work policy?"
    actual_provider = embedded_chunks[0]['metadata']['provider']  # Use provider that actually worked
    
    query_result = embed_documents(
        [query],
        chunk_size=256,
        provider=actual_provider,  # Use same provider as documents
    )
...
def demo_provider_fallback():
    ...
    print("\nSupported embedding providers (Cohere is default):")
    print("  • Cohere - fast, reliable semantic embeddings (primary)")
    print("  • OpenRouter (Jina Embeddings v3) - fallback when Cohere unavailable\n")
